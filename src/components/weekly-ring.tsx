interface Props {
  ...
  size?: number;
  dots?: boolean[];
  /** Optional content rendered below the ring + dots — meant for streak/weather chip. */
  footerSlot?: React.ReactNode;
}

export function WeeklyRing({ minutes, goal = 90, size = 96, dots, footerSlot }: Props) {
  const stroke = 8;
  ...
  return (
    <div className="space-y-3">
      ...
      {footerSlot && (
        <div className="-mx-1 border-t border-border/50 pt-3">
          {footerSlot}
        </div>
      )}
    </div>
