import { Footprints, Share2, Camera } from "lucide-react";
 ...
 */
export function EntryCard({ walk, snapshotUrl, photoCount = 0, contextLine, active, onSelect, onShare }: Props) {
  const w = walk;
    ...
    <article className={`group relative overflow-hidden rounded-3xl border transition active:scale-[0.995] ${active ? "border-forest shadow-elevated" : "border-border bg-card shadow-soft hover:border-forest/30"}`}>
      ...
      >
        ...
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-secondary/40">
          ...
          {/* Duotone overlay for legibility */}
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-foreground/55 via-foreground/10 to-transparent" />
          {/* Top meta */}
          <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-3 text-cream/95">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] [text-shadow:0_1px_2px_rgba(0,0,0,0.35)]">
              {dateLabel} · {timeLabel}
            </div>
          ...
          {/* Stat trio overlaid on bottom of map */}
          <div className="absolute inset-x-0 bottom-0 flex items-baseline justify-between gap-3 p-4 text-cream">
        ...
        <div className="space-y-2 px-4 pb-4 pt-3">
          {contextLine && (
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{contextLine}</div>
          )}
          ...
          {photoCount > 0 && (
            <div className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
              <Camera className="h-3 w-3" /> {photoCount} {photoCount === 1 ? "photo" : "photos"}
            </div>
          )}
        </div>
      </button>
...
function Stat({ value, unit }: { value: string; unit: string }) {
  return (
    <div className="flex items-baseline gap-1 [text-shadow:0_1px_3px_rgba(0,0,0,0.45)]">
