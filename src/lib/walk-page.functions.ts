...
export const getWalkByCode = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: event, error } = await supabaseAdmin
      .from("events")
      .select(
        "id,slug,title,description,starts_at,ends_at,timezone,venue_name,address,city,lat,lng,vibe,visibility,host_user_id,attendee_count,image_url,cover_override_url,meeting_point,accessibility_notes,event_type,place_id,group_id,circle_id,pace,distance_meters,dog_friendly,kid_friendly,is_seed,seed_schedule_id"
      )
      .eq("slug", data.code)
