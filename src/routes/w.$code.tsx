...
import { Footprints, Headphones, Mic } from "lucide-react";
...
function FriendWalkLanding() {
  ...
  const [room, setRoom] = useState<{ title: string; speakers: number; cap: number; status: string; host?: { display_name: string | null; avatar_url: string | null } } | null>(null);
  ...
  useEffect(() => {
    ...
    supabase
      .from("audio_rooms")
      .select("title, current_participant_count, max_participants, status, host_user_id")
  ...
  const ended = room && room.status !== "open";
  const full = room && room.speakers >= room.cap;
  ...
  return (
    <div className="mx-auto -mx-4 min-h-[80vh] gradient-forest px-5 py-12 text-primary-foreground md:mx-0 md:rounded-3xl">
      <div className="mx-auto max-w-md text-center">
        ...
        <p className="text-[10px] uppercase tracking-[0.32em] opacity-80">live walk · link only</p>
        <h1 className="mt-2 font-serif text-3xl italic leading-tight">
          {room?.host?.display_name ? `${room.host.display_name} is walking` : room?.title || "a friend's walk"}
        </h1>
        <p className="mt-3 text-sm opacity-85">come walk with them — talk, listen, or just be near.</p>
        ...
        {room && !ended && (
          <div className="mt-10 space-y-3">
            ...
            >
              ...
              {full ? "Speakers full — join as listener" : "Join the walk · talk"}
            </Button>
            <Button
              onClick={() => goJoin(true)}
              disabled={busy}
            ...
            <p className="pt-2 text-[11px] opacity-70">
              ...
              {room.speakers} on the walk · 4 mics
            </p>
          </div>
        )}
      </div>
